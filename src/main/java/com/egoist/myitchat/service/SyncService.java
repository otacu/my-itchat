...
import com.egoist.myitchat.enums.RetCode;
import com.egoist.myitchat.enums.Selector;
import com.egoist.myitchat.pojo.response.SyncCheckResponse;
import com.egoist.myitchat.pojo.response.SyncResponse;
import com.egoist.myitchat.pojo.response.VerifyUserResponse;
...
public class SyncService {
        ...
        while (true) {
            try {
                ...
                if (retCode == RetCode.NORMAL.getCode()) {
                    ...
                    logger.error("syncCheckResponse selector = " + selector);
                }

            } catch (Exception e) {
                logger.error("listen error:" + e.toString());
            } finally {
                try {
    ...
    private boolean isMessageFromChatRoom(Message message) {
        return message.getFromUserName() != null && (message.getFromUserName().startsWith("@@") || message.getToUserName().startsWith("@@"));
    }
    ...
    private void onNewMessage() throws IOException, URISyntaxException {
        ...
        for (Message message : syncResponse.getAddMsgList()) {
            ...
            if (message.getMsgType() == MessageType.TEXT.getCode()) {
                ...
                cacheService.getContactNamesWithUnreadMessage().add(message.getFromUserName());
                String fullImageUrl = String.format(WechatConstants.WECHAT_URL_GET_MSG_IMG, cacheService.getHostUrl(), message.getMsgId(), cacheService.getsKey());
